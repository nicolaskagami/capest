...
def main():
    ...
    #subprocess.Popen('sudo ../sink/capestd 6666 >> ./.capestd & ',shell=True)
    sleep(60)

    def setupLinks(capacity):
        for interface in ['s1-eth1', 's1-eth2', 's2-eth1', 's2-eth2', 's2-eth3', 's3-eth1', 's3-eth2', 's3-eth3','s4-eth1']:
    ...
    def runTest(method,nom_cap,xtraffic_type,xtraffic_cap):
        setupLinks(nom_cap)
        print "Running X-Traffic Test: " + str(method) + "," + str(nom_cap) 
        ...
        if xtraffic_type == 0:
            h1.cmd('iperf3 -s &')
            h2.cmd('iperf3 -c 10.0.1.10 -b %sK -t 60 & > ./iperf' % xtraffic_cap)
        elif xtraffic_type == 1:
            h2.cmd('iperf3 -s &')
            h3.cmd('iperf3 -c 10.0.2.10 -b %sK -t 60 & > ./iperf' % xtraffic_cap)
        elif xtraffic_type == 2:
            h1.cmd('iperf3 -s &')
            h3.cmd('iperf3 -c 10.0.1.10 -b %sK -t 60 & > ./iperf' % xtraffic_cap)
        elif xtraffic_type == 3:
            h2.cmd('iperf3 -s &')
            h1.cmd('iperf3 -c 10.0.2.10 -b %sK -t 60 & > ./iperf' % xtraffic_cap)

        sleep(30)
        h2.cmd('ping 10.0.4.10 -i 0.2 -c 75 -q | tail -n 2 > .pingdata &')
        sleep(30)
        capest.kill()
        call('./gatherXTrafficData.sh %s %s %s %s' % (method,nom_cap,xtraffic_type,xtraffic_cap), shell=True)
    ...
    def runBatch(repetitions):
        for x in range(0,repetitions):
            runTest("XTraffic_"+args.json,10000,4,10)
            runTest("XTraffic_"+args.json,10000,4,10)
            runTest("XTraffic_"+args.json,10000,4,10)
            runTest("XTraffic_"+args.json,10000,4,10)
            runTest("XTraffic_"+args.json,10000,0,2000)
            runTest("XTraffic_"+args.json,10000,0,4000)
            runTest("XTraffic_"+args.json,10000,0,6000)
            runTest("XTraffic_"+args.json,10000,0,8000)
            runTest("XTraffic_"+args.json,10000,0,10000)
            runTest("XTraffic_"+args.json,10000,1,2000)
            runTest("XTraffic_"+args.json,10000,1,4000)
            runTest("XTraffic_"+args.json,10000,1,6000)
            runTest("XTraffic_"+args.json,10000,1,8000)
            runTest("XTraffic_"+args.json,10000,1,10000)
            runTest("XTraffic_"+args.json,10000,2,2000)
            runTest("XTraffic_"+args.json,10000,2,4000)
            runTest("XTraffic_"+args.json,10000,2,6000)
            runTest("XTraffic_"+args.json,10000,2,8000)
            runTest("XTraffic_"+args.json,10000,2,10000)
            runTest("XTraffic_"+args.json,10000,3,2000)
            runTest("XTraffic_"+args.json,10000,3,4000)
            runTest("XTraffic_"+args.json,10000,3,6000)
            runTest("XTraffic_"+args.json,10000,3,8000)
            runTest("XTraffic_"+args.json,10000,3,10000)
