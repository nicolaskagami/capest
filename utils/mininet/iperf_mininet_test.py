...
def main():
    ...
    call('sudo /home/nicolassk/capest/sink/sink s4-eth2 127.0.0.1 6666 &',shell=True)
    sleep(60)
    ...
    def runTest(method,nom_cap,num_videos):
        ...
        capest = subprocess.Popen('sudo script -c "/home/nicolassk/capest/sink/capestd 6666" -f /home/nicolassk/capest/INT/.capestd &',shell=True)
        ...
        #h3.cmd('killall -9 iperf3')
        #h3.cmd('iperf3 -s &')
        h1.cmd('iperf -c 10.0.3.10 -f k -y c -t 60 &> .iperf &')
        h2.cmd('ping 10.0.4.10 -i 1 -c 60 -q -s 0 > /dev/null &')
        sleep(480)
        sleep(120)
        capest.kill()
        call('./gatherIperfData.sh %s %s %s' % (method,nom_cap,num_videos), shell=True)
    ...
    def runBatch(repetitions):
        for x in range(0,repetitions):
            #runTest("Video_Iperf"+args.json,4000,1)
            #runTest("Video_Iperf"+args.json,6000,1)
            #runTest("Video_Iperf"+args.json,6000,2)
            #runTest("Video_Iperf"+args.json,8000,1)
            #runTest("Video_Iperf"+args.json,8000,3)
            #runTest("Video_Iperf"+args.json,10000,2)
            #runTest("Video_Iperf"+args.json,10000,4)
            #runTest("Video_Iperf"+args.json,12000,1)
            #runTest("Video_Iperf"+args.json,12000,5)
            #runTest("Video_Iperf"+args.json,14000,1)
            #runTest("Video_Iperf"+args.json,14000,5)
            #runTest("Video_Iperf"+args.json,14000,6)
            runTest("Video_Iperf"+args.json,16000,1)
            runTest("Video_Iperf"+args.json,16000,6)
            runTest("Video_Iperf"+args.json,16000,7)
