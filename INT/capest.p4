...
parser ParserImpl(packet_in packet,out headers hdr,inout metadata meta,inout standard_metadata_t standard_metadata) 
{
    ...
    state parse_capest 
    {
        packet.extract(hdr.capest);
        meta.parser_metadata.remaining = hdr.capest.count;
...
control egress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) 
{
    ...
    action extraction_procedure(switchID_t id) 
    {    
        //Variables
        bit<48> last_pkt_ts;
    ...
    action probe_procedure(switchID_t id) 
    {    
        //Variables
        bit<32> estimate;
        bit<32> index;
        ...
        hdr.capest.count = hdr.capest.count + 1;
    ...
    action remove_capest()
    {
        hdr.nsh.setInvalid(); 
        hdr.nsh_context.setInvalid(); 
        hdr.capest.setInvalid(); 
        hdr.ethernet.etherType = ETH_TYPE_IPV4;
        hdr.caps.pop_front(hdr.capest.count);
    }
    table live 
    {
        actions        = { extraction_procedure; NoAction; }
        default_action =  NoAction();      
    }
    table probe 
    {
        actions        = { probe_procedure; NoAction; }
        default_action =  NoAction();      
    }
