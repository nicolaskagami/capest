...
let 'i = i+ 1'
CONVERGENCE_ROUND=$i
ROUNDS_PER_MBPS=$(bc <<< "scale=6;$CONVERGENCE_ROUND*1000/(1+${ROUNDS[$CONVERGENCE_ROUND]}-${ROUNDS[0]})")

rounds=$(printf ",%s" "${ROUNDS[@]}")
echo "$METHOD,$NOM_CAPACITY,$MEASURED_CAPACITY,$CONVERGENCE_ROUND,$ROUNDS_PER_MBPS,$LATENCY,$JITTER,$LOSS$rounds" >> Factor_Results.dat
